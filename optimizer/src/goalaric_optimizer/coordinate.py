...
def _parameter_specs(registry: Registry) -> tuple[ParameterSpec, ...]:
    ...
    for item in registry.parameters:
        ...
        if any(isinstance(value, bool) or not isinstance(value, int) for value in (minimum, maximum, step)):
            raise ValidationError(f"coordinate bounds for {name} must be integers")
        if step <= 0 or minimum > maximum or not minimum <= default <= maximum:
            raise ValidationError(f"invalid coordinate bounds for {name}")
        specs.append(ParameterSpec(name, minimum, maximum, step))
    return tuple(specs)
