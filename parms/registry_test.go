...
func TestLMPDepthRegistryRoundTripsAndApplies(t *testing.T) {
	original := Search
	t.Cleanup(func() { Search = original })

	registry := LMPDepthRegistry()
	if len(registry) != 1 {
		t.Fatalf("LMP-depth registry has %d parameters, want 1", len(registry))
	}
	got := registry[0]
	if got.Name != "lmp_depth4_min_iteration" || got.Default != 0 || got.Min != 0 || got.Max != 20 || got.Step != 2 {
		t.Fatalf("LMP-depth descriptor = %+v", got)
	}

	want, err := DefaultLMPDepthParameterJSON()
	if err != nil {
		t.Fatal(err)
	}
	parsed, err := ParseParameterFile(want)
	...
	parsed.Parameters[0].Value = 12
	if err := ApplyParameterFile(parsed); err != nil {
		t.Fatal(err)
	}
	if Search.LMPDepth4MinIteration != 12 {
		t.Fatalf("LMP depth-4 activation=%d, want 12", Search.LMPDepth4MinIteration)
	}
	if Search.LMPMoveMultiplier != original.LMPMoveMultiplier || Search.LMRDivisorX100 != original.LMRDivisorX100 {
		t.Fatal("search-lmp-depth-v1 changed unrelated search parameters")
	}
}

func TestCheckedInDefaultLMPDepthParameterFileMatchesExporter(t *testing.T) {
	_, source, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("runtime.Caller failed")
	}
	path := filepath.Join(filepath.Dir(source), "..", "optimizer", "registries", "search-lmp-depth-v1-default.json")
	checkedIn, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
	exported, err := DefaultLMPDepthParameterJSON()
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(checkedIn, exported) {
		t.Fatalf("checked-in LMP-depth file differs from exporter:\n%s\nwant:\n%s", checkedIn, exported)
	}
}

func TestLMPDepthParameterFileRejectsInvalidRangeAndStep(t *testing.T) {
	file := DefaultLMPDepthParameterFile()
	file.Parameters[0].Value = -2
	if _, err := MarshalParameterFile(file); err == nil {
		t.Fatal("negative LMP-depth activation was accepted")
	}
	file.Parameters[0].Value = 11
	if _, err := MarshalParameterFile(file); err == nil {
		t.Fatal("odd LMP-depth activation was accepted")
	}
	file.Parameters[0].Value = 22
	if _, err := MarshalParameterFile(file); err == nil {
		t.Fatal("too-large LMP-depth activation was accepted")
	}
}
