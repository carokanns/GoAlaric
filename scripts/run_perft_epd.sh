...
# Användning: ./scripts/run_perft_epd.sh <djup> <epd-fil> [utfil]
...
echo -e "nodes\tms\tnps" >"$output_file"
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  printf "%s\t%d\t%s\n" "$nodes" "$duration_ms" "$nps" >>"$output_file"
...
if [ "$row_count" -gt 0 ]; then
  ...
  printf "avg\t%s\t%s\t%s\n" "$avg_nodes" "$avg_ms" "$total_nps" >>"$output_file"
fi
