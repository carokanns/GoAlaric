package castling

import (
	"GoAlaric/hash"
	"GoAlaric/square"
	"fmt"
)

type info struct {
	KingFr,
	KingTo,
	RookFr,
	RokTo int
}

// Info is the castling Info
var Info = [...]info{{square.E1, square.G1, square.H1, square.F1},
	{square.E1, square.C1, square.A1, square.D1},
	{square.E8, square.G8, square.H8, square.F8},
	{square.E8, square.C8, square.A8, square.D8},
}

// Flags for castling
var (
	FlagsMask [square.BoardSize]int
	FlagsKey  [1 << 4]hash.Key
)

// Index of the castling
// sd: side to move, wg: castle side - short or long (that is king side or queen side)
func Index(sd, wg int) int {
	return sd*2 + wg
}

// Side returns which side(s) can be castled to
func Side(index int) int {
	return index / 2
}

// Flag returns one of the castling flags
func Flag(flags int, index uint) bool {
	//assert(index < 4);
	return ((flags >> index) & 1) != 0
}

// SetFlag sets one of the castling flags
func SetFlag(flags *int, index uint) {
	//assert(index < 4);
	*flags |= (1 << index)
}

// Init the castling flags
func init() {
	fmt.Println("info string Castling init startar")
	for sq := 0; sq < square.BoardSize; sq++ {
		FlagsMask[sq] = 0
	}

	SetFlag(&FlagsMask[square.E1], 0)
	SetFlag(&FlagsMask[square.E1], 1)
	SetFlag(&FlagsMask[square.H1], 0)
	SetFlag(&FlagsMask[square.A1], 1)

	SetFlag(&FlagsMask[square.E8], 2)
	SetFlag(&FlagsMask[square.E8], 3)
	SetFlag(&FlagsMask[square.H8], 2)
	SetFlag(&FlagsMask[square.A8], 3)

	for flags := 0; flags < (1 << 4); flags++ {
		FlagsKey[flags] = computeFlagsKey(flags)
	}
}
func computeFlagsKey(flags int) hash.Key {

	key := hash.Key(0)

	for index := uint(0); index < 4; index++ {
		if Flag(flags, index) {
			key ^= hash.FlagKey(index)
		}
	}

	return key
}
